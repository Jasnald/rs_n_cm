...
class ContourAnalysisExtended(LoggerMixin, object):
    ...
    def mesh_tree(self):
        ...
        self._datum.xz(cordY)

        self._cell.set(name='ALMA', yMin=cordY)
        self._cell.set(name='BASE', yMax=cordY)

        # Partições YZ (Cortes verticais ao longo de X)
        self._cut.coord = cord0
        self._datum.yz(cord0, 'BASE')
        self._datum.yz(cordL, 'BASE')

        self._cell.set(name='BASE_I', yMax=cordY, xMin=cord0, xMax=cordL)
        self._datum.xz(cordY - 2, 'BASE_I')

        self._datum.yz(cord1, 'ALMA')
        self._datum.yz(cord2, 'ALMA')
        self._datum.yz(cord3, 'ALMA')
        ...
        self._edge.set(name='Edge_bdr_o',
                       xMin= - o3, xMax=25 + o3,
                       yMax=cordY + o3, yMin= -o3)

        self._edge.set(name='Edge_bdr_i',
                       xMin=cord0 - o3, xMax=cordL + o3,
                       yMax=cordY, yMin= cordY - ofs2 -o3)
        self._edge.set(name='Edge_bdr_i2',
                       xMin=cord0 - o3, xMax=cordL + o3,
                       yMax=cordY-ofs2-o3, yMin= -o3)
        '''
        ...
        self._cell.set(name='Cell_Sweep_1',
                       xMin=cord0 , xMax=cordL,
                       yMin=cordY , yMax=cordY)

        self._cell.set(name='Cell_Sweep',
                       yMax=cordY-0.5,
                       yMin=cordY- ofs2,
                       xMin=cord0, xMax=cordL
                       )
        ...
        self._cell.set(name='SHL_Rgn_1',
                       xMax=cord1, yMin=cordY)

        self._cell.set(name='SLD_Rgn_2',
                       xMin=cord1, xMax=cord2, yMin=cordY)

        self._cell.set(name='SHL_Rgn_2',
                       xMin=cord2, xMax=cord3, yMin=cordY)

        self._cell.set(name='SHL_Rgn_3',
                       xMin=cord3, xMax=cordL, yMin=cordY)


        # -------------------------------------------------------------
        # SEMENTES DE MALHA
        # -------------------------------------------------------------
        self._mesher_seed.mesh(edge_set_name='Edge_bdr_o', num_elements=30)
        self._mesher_seed.mesh(edge_set_name='Edge_bdr_i', num_elements=10)
        self._mesher_seed.mesh(edge_set_name='Edge_bdr_i2', num_elements=15)
        ...
        '''
        self._mesher_seed.mesh(edge_set_name='Edge_bdr_3R', num_elements=4)
        self._mesher_seed.mesh(edge_set_name='Edge_bdr_3L', num_elements=4)
        self._mesher_seed.mesh(edge_set_name='Edge_bdr_3C', num_elements=1)

        self._mesher_seed.mesh(edge_set_name='Edge_swp_1', num_elements=20)
        self._mesher_seed.mesh(edge_set_name='Edge_swp_2', num_elements=20)
        '''

        #self._mesher_sweep.set(cell_set_name='Cell_Sweep_1')
        #self._mesher_sweep.set(cell_set_name='Cell_Sweep_2')
        #self._mesher_sweep.set(cell_set_name='Cell_Sweep')
    ...
    def run_extended_analysis(self):
        ...
        self._section.create(
            self.mat_name, 'SLD_Rgn_2', self.section_name + "_1")
        self._section.create(
            self.mat_name, 'BASE', self.section_name + "_2")

        # 2. Seção Shell Esquerda (Thickness = cord1)
        ipts = 11
        self._section.create(
            self.mat_name, 'SHL_Rgn_1', self.section_name + "Shell1",
            False, self.cord1, ipts)
        ...
        self._section.create(
            ...
            False, shl_rgn_32_thickness, ipts)
